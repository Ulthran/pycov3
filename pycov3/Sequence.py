...
class Contig(Sequence):
    def __init__(
        ...
        if not (10 <= self.window_step <= 1000):
            logging.error(
                f"Window step of {self.window_step} is not between 10 and 1,000, please choose a value in this range"
            )
            raise ValueError
        if not (500 <= self.window_size <= 10000):
            logging.error(
                f"Window size of {self.window_size} is not between 500 and 10,000, please choose a value in this range"
            )
            raise ValueError
